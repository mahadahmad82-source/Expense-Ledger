      ...
      {!selectedLedger ? (
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 rounded-2xl bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 px-3 py-1.5 text-xs text-slate-600 dark:text-slate-300">
              ...
              return (
                <div
                  ...
                  className={`group relative overflow-hidden rounded-3xl p-6 border transition-all cursor-pointer backdrop-blur-xl shadow-xl hover:scale-[1.02] ${
                    isCurrent
                      ? 'bg-gradient-to-tr from-purple-100/80 via-white/90 to-indigo-100/80 dark:from-purple-950/70 dark:via-slate-900/80 dark:to-indigo-950/70 border-purple-500/40 shadow-purple-500/10'
                      : 'bg-white/80 dark:bg-slate-900/50 border-slate-200 dark:border-white/10 hover:border-purple-400 dark:hover:border-white/20'
                  }`}
                >
                  {/* Status Pill */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div className={`rounded-xl p-2 text-white shadow-sm ${isCurrent ? 'bg-purple-600' : 'bg-slate-700'}`}>
                        <Calendar className="h-4 w-4" />
                      </div>
                      <h3 className="text-base font-bold text-slate-900 dark:text-white font-display">{ledger.month_name}</h3>
                    </div>
                  ...
                  {/* Financial Stats */}
                  <div className="grid grid-cols-2 gap-3 py-3 border-y border-slate-100 dark:border-white/10 my-3">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Total Income</p>
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Total Outflow</p>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div>
                      <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">Net Month Savings:</span>
                      <p className={`text-base font-black font-mono ${ledger.net_savings >= 0 ? 'text-purple-600 dark:text-purple-300' : 'text-rose-600 dark:text-rose-400'}`}>
                        {formatPKR(ledger.net_savings)}
                      </p>
                    </div>

                    <div className="flex items-center gap-1 text-xs font-bold text-purple-600 dark:text-purple-400 group-hover:underline">
                      <span>Inspect</span>
        ...
        <div className="space-y-5 animate-in fade-in duration-200">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-2xl bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-2.5 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors shadow-sm"
              ...
              <button
                ...
                className={`flex items-center gap-1.5 rounded-2xl border px-3.5 py-2 text-xs font-semibold transition-colors ${
                  selectedLedger.is_closed
                    ...
                    : 'bg-slate-100 dark:bg-white/10 border-slate-200 dark:border-white/15 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/15'
                }`}
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-3xl bg-white/80 dark:bg-slate-900/60 border border-emerald-500/30 p-4 backdrop-blur-xl shadow-md">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Inflow (Income)</span>
              <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400 font-mono mt-1">
                {formatPKR(selectedLedger.total_income)}
              </p>
            </div>

            <div className="rounded-3xl bg-white/80 dark:bg-slate-900/60 border border-rose-500/30 p-4 backdrop-blur-xl shadow-md">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Outflow (Expenses)</span>
              <p className="text-xl font-bold text-rose-600 dark:text-rose-400 font-mono mt-1">
                {formatPKR(selectedLedger.total_expense)}
              </p>
            </div>

            <div className="rounded-3xl bg-white/80 dark:bg-slate-900/60 border border-purple-500/30 p-4 backdrop-blur-xl shadow-md">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Net Month Surplus</span>
              <p className={`text-xl font-bold font-mono mt-1 ${selectedLedger.net_savings >= 0 ? 'text-purple-600 dark:text-purple-300' : 'text-rose-600 dark:text-rose-400'}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 rounded-2xl bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 shadow-md">
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 pl-9 pr-3 py-1.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>

            <div className="flex items-center gap-2 w-full sm:w-auto">
              <div className="flex rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-0.5">
                {(['all', 'expense', 'income'] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => setTableTypeFilter(t)}
                    className={`rounded-lg px-3 py-1 text-[11px] font-bold capitalize transition-colors ${
                      tableTypeFilter === t ? 'bg-purple-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                    }`}
                  >
                    {t}
                  </button>
          ...
          <div className="rounded-3xl bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 backdrop-blur-xl shadow-xl overflow-hidden transition-colors">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                    <th
                    ...
                    <th
                      className="px-4 py-3.5 text-right cursor-pointer hover:text-slate-900 dark:hover:text-white"
                      onClick={() => {
                        setSortField('amount');
                        setSortOrder(sortOrder === 'desc' ? 'asc' : 'desc');
                      }}
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-slate-700 dark:text-slate-300 font-sans">
                  {monthTransactions.length === 0 ? (
                    ...
                    monthTransactions.map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors font-mono">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span
                              className={`rounded-md px-2 py-0.5 text-[10px] font-bold font-sans uppercase ${
                                isIncome ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : 'bg-rose-500/20 text-rose-700 dark:text-rose-300'
                              }`}
                            >
                              {t.type}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-sans font-semibold text-slate-900 dark:text-white whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: cat?.color || '#8B5CF6' }} />
                              <span>{cat?.name || 'Uncategorized'}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 font-sans text-slate-600 dark:text-slate-400 max-w-xs truncate">
                            {t.note || '-'}
                          </td>
                          <td className="px-4 py-3 font-sans text-slate-700 dark:text-slate-300 whitespace-nowrap">
                            {wallet?.name || 'Wallet'}
                          </td>
                          <td className={`px-4 py-3 text-right font-black whitespace-nowrap ${isIncome ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-slate-100'}`}>
