...
export const TransactionsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-8">
      ...
      <div className="rounded-3xl bg-slate-900/60 border border-white/10 p-4 sm:p-5 backdrop-blur-xl shadow-xl space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div className="md:col-span-5 relative flex items-center">
            <Search className="absolute left-3.5 h-4 w-4 text-purple-400" />
            <input
              type="text"
              placeholder="Search category, note, amount, wallet..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full rounded-2xl bg-white/5 border border-white/10 pl-10 pr-4 py-2 text-xs text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>

          {/* Type Toggle (4 cols) */}
          <div className="md:col-span-4 flex rounded-2xl bg-white/5 border border-white/10 p-1">
          ...
          <div className="md:col-span-3">
            <select
              ...
              className="w-full rounded-2xl bg-slate-800 border border-white/10 px-3 py-2 text-xs font-semibold text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-white/5">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl bg-slate-800 border border-white/10 px-2.5 py-1.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          ...
          <div>
            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">Wallet</label>
            <select
          ...
          <div>
            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">From Date</label>
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="w-full rounded-xl bg-slate-800 border border-white/10 px-2 py-1.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>

          <div>
            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">To Date</label>
            <input
              type="date"
              value={endDate}
        ...
        <div className="flex items-center justify-between pt-2 text-xs">
          <div className="flex items-center gap-3">
            <span className="text-slate-400">
              Showing <span className="font-bold text-white">{filteredSummary.count}</span> records
            </span>
            <span className="text-slate-500">•</span>
            <span className="text-emerald-400 font-semibold">+{formatPKR(filteredSummary.inc)}</span>
            <span className="text-rose-400 font-semibold">-{formatPKR(filteredSummary.exp)}</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-xl px-3 py-1.5 text-xs font-semibold border transition-colors ${
              isBulkMode ? 'bg-purple-600/30 border-purple-500 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:text-white'
            }`}
          >
      ...
      <div className="rounded-3xl bg-slate-900/60 border border-white/10 backdrop-blur-xl shadow-xl overflow-hidden">
        {filteredTransactions.length === 0 ? (
          <div className="py-16 text-center text-slate-500">
            <Tag className="h-10 w-10 mx-auto mb-2 opacity-30 text-purple-400" />
            <p className="text-sm font-semibold text-slate-400">No transactions match the criteria</p>
            <p className="text-xs text-slate-500 mt-1">Try resetting the filters or logging a new entry.</p>
          </div>
        ) : (
            ...
            {filteredTransactions.map((tx) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-4 sm:px-6 hover:bg-white/5 transition-colors cursor-pointer ${
                    isSelected ? 'bg-purple-600/15' : ''
                  }`}
                >
                  <div className="flex items-center gap-3.5 truncate">
                    ...
                    <div
                      className="flex h-11 w-11 items-center justify-center rounded-2xl text-white shrink-0"
                      style={{
                    ...
                    <div className="truncate">
                      <div className="flex items-center gap-2">
                        ...
                        {tx.is_recurring && (
                          <span className="flex items-center gap-0.5 rounded-full bg-purple-500/20 px-1.5 py-0.2 text-[9px] font-semibold text-purple-300 border border-purple-500/30">
                        ...
                        {tx.receipt_url && (
                          <span
                            ...
                            className="rounded-md bg-purple-500/20 px-1.5 py-0.5 text-[9px] font-semibold text-purple-300 hover:bg-purple-500/40"
                          >
                            📷 Receipt
                          </span>
                        )}
                      ...
                      <p className="text-[11px] text-slate-400 truncate mt-0.5">
                        ...
                        {' • '}
                        <span className="text-slate-300 font-medium">{wallet?.name || 'Wallet'}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0 pl-3">
                    <p
                      className={`text-sm sm:text-base font-black font-mono ${
                        isIncome ? 'text-emerald-400' : isTransfer ? 'text-indigo-300' : 'text-slate-100'
                      }`}
                    >
                      {isIncome ? '+' : isTransfer ? '↔ ' : '-'}{formatPKR(tx.amount)}
                    </p>
                    <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5">{formatDate(tx.date)}</p>
                  </div>
                </div>
              );
            })}
