  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="rounded-3xl bg-slate-900/60 border border-amber-500/30 p-5 backdrop-blur-xl shadow-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-amber-400">Pending Dues (This Cycle)</span>
            <p className="text-2xl font-black text-amber-300 font-mono mt-1">{formatPKR(totalPending)}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">{bills.filter(b => !b.is_paid).length} bills awaiting payment</p>
          </div>
        ...
        <div className="rounded-3xl bg-slate-900/60 border border-emerald-500/30 p-5 backdrop-blur-xl shadow-xl flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-400">Cleared & Paid</span>
            <p className="text-2xl font-black text-emerald-300 font-mono mt-1">{formatPKR(totalPaid)}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">{bills.filter(b => b.is_paid).length} bills successfully cleared</p>
          </div>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white">{bill.title}</h3>
                    <p className="text-[10px] text-slate-400">{cat?.name || 'Utility'}</p>
                  </div>
                </div>
              ...
              {/* Amount & Due Date */}
              <div className="my-4 py-2 border-y border-white/5 flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-bold text-slate-400">Bill Amount</span>
                  <p className="text-lg font-black text-white font-mono">{formatPKR(bill.amount)}</p>
                </div>

                <div className="text-right">
                  <span className="text-[10px] uppercase font-bold text-slate-400">Due Date</span>
                  <p className={`text-xs font-bold ${isOverdue ? 'text-rose-400' : isDueToday ? 'text-amber-300' : 'text-slate-300'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-sm rounded-3xl bg-slate-900 border border-white/15 p-6 shadow-2xl backdrop-blur-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-white/10 mb-4">
              <h3 className="text-base font-bold text-white">Record Bill Payment</h3>
              <button
            ...
            <p className="text-xs text-slate-300 mb-1 font-semibold">{payingBill.title}</p>
            <p className="text-xl font-black text-emerald-400 font-mono mb-4">{formatPKR(payingBill.amount)}</p>
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-md rounded-3xl bg-slate-900 border border-white/15 p-6 shadow-2xl backdrop-blur-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-white/10 mb-4">
              <h3 className="text-base font-bold text-white">Add Bill Reminder</h3>
              <button
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl bg-slate-800 border border-white/15 px-2.5 py-2 text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>
              </div>
