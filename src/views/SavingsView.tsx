...
export const SavingsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-8">
      ...
      <div className="rounded-3xl bg-gradient-to-tr from-emerald-950/70 via-slate-900/80 to-purple-950/70 border border-emerald-500/30 p-6 backdrop-blur-xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-300">Total Saved Across Targets</span>
          <h2 className="text-3xl sm:text-4xl font-black text-white font-display mt-1">
            {formatPKR(totalSaved)}
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Overall Progress: <span className="font-bold text-emerald-400">{overallProgress}%</span> of {formatPKR(totalTarget)} combined goal
          </p>
        </div>
          ...
          return (
            <div
              ...
              className={`rounded-3xl p-6 border backdrop-blur-xl shadow-xl relative overflow-hidden transition-all hover:scale-[1.01] ${
                isCompleted
                  ? 'bg-gradient-to-tr from-emerald-950/50 via-slate-900/80 to-teal-950/50 border-emerald-500/50 shadow-emerald-500/10'
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-2xl text-white ${
                      isCompleted ? 'bg-emerald-500 shadow-lg shadow-emerald-500/30' : 'bg-purple-600/30 text-purple-300'
                    }`}
                  >
                    {isCompleted ? <Award className="h-6 w-6 text-white" /> : <Target className="h-6 w-6 text-purple-400" />}
                  </div>
                  <div>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-baseline justify-between">
                  <span className="text-xl font-black text-white font-mono">{formatPKR(goal.current_amount)}</span>
                  <span className="text-xs font-bold text-slate-400 font-mono">/ {formatPKR(goal.target_amount)}</span>
                </div>

                <div className="h-2.5 w-full rounded-full bg-white/10 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      isCompleted ? 'bg-emerald-400' : 'bg-gradient-to-r from-purple-500 to-indigo-400'
                    }`}
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-white/10">
                <button
                  ...
                  className="flex-1 rounded-2xl bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 py-2 text-xs font-bold text-emerald-300 text-center transition-colors"
                >
                  + Add Funds
                </button>
                ...
                <button
                  ...
                  className="rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 transition-colors"
                >
                  Withdraw
                </button>
              </div>
            ...
            <form onSubmit={handleCreateGoal} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-2xl bg-white/5 border border-white/15 px-3.5 py-2 text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
              </div>
